...
def time_limit(seconds: int):
    """Raise TimeoutError if the body takes longer than *seconds*. Linux only."""
...
def math_seq(operation: str, ctx: MathEvalContext,
    ...
    if operation == "seq":
        if int(end) - int(start) > 10_000:
            raise ValueError(f"seq range too large: {int(end) - int(start)}")
        results = []
        ...
        if r is None:
            r = len(values)
        if perm(len(values), r) > 100_000:
            raise ValueError(f"perm too large: P({len(values)},{r}) > 100_000")
        results = []
        ...
        if r is None:
            r = len(values)
        if comb(len(values), r) > 100_000:
            raise ValueError(f"comb too large: C({len(values)},{r}) > 100_000")
        results = []
...
def run_tool_calls(tool_call_json: str, ctx: "MathEvalContext | None" = None) -> list:
    """Execute a JSON array of tool calls in order, return list of results.

    Pass an existing *ctx* to share variable state across multiple rounds
    (e.g. variables set in round 1 remain visible in round 2).
    """
